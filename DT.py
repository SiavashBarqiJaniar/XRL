...
def get_instances(startpnt, endpnt):
    ...
    for iteration in range(startpnt, endpnt+1):
        dataset_file = f'trajectories/{modelName}/game{iteration}.csv'
...
if __name__ == '__main__':
    ...
    for c in range(10):
        if True:
            x, y = get_instances(50, 50) #18+ iterations-1)
            #print('data:', len(x), len(y))
        ...
        ###### Accuracy
        x_test, y_test = get_instances(51, 51)
        ...
        for r in range(len(y_test)):
            prob_dist = find_probability_distribution(r, x_test, y_test) # (A)
            errors_MSE.append(MSE(prob_dist, y_pred[r]))
            errors_MAE.append(MAE(prob_dist, y_pred[r]))
        ...
        with open(f'report/{modelName}.txt', 'a') as f:
    ...
    tree.export_graphviz(model, feature_names=features, filled=True, out_file=f"treeStrustures/DT_{modelName}.dot", rounded=True, node_ids=True, proportion=True)
